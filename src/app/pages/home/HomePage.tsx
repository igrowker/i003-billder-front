...
export const HomePage = React.memo(() => {
  ...
  const { getClients, clients } = useClientStore(useShallow(state => ({ getClients: state.getClients, clients: state.clients })));
    ...
    return (
      <div className="  p-4 flex flex-col  ">
        ...
        <div className="">
          ...
          {clients.length > 0 ? (
            clients.map((client) => (
              <ClientCard onClick={() => navigate(`client/${client.id}`)} client={client} key={client.id} />
            ))
          ) : (
